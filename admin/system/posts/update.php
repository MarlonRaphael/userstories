            ...
            <div class="label_line">
                ...
                <label class="label_small">
                    ...
                    <select name="post_category">
                        ...
                        <?php
                        $readSes = new Read;
                        $readSes->ExeRead("categoria", "WHERE category_parent IS NULL ORDER BY category_title ASC");
                        if ($readSes->getRowCount() >= 1):
                            foreach ($readSes->getResult() as $ses):
                                echo "<option disabled=\"disabled\" value=\"\"> {$ses['category_title']} </option>";
                                $readCat = new Read;
                                $readCat->ExeRead("categoria", "WHERE category_parent = :parent ORDER BY category_title ASC", "parent={$ses['category_id']}");
                ...
                <label class="label_small">
                    ...
                    <select name="post_author">
                        ...
                        <?php
                        $readAut = new Read;
                        $readAut->ExeRead("usuarios", "WHERE user_id != :id AND user_level >= :level ORDER BY user_name ASC", "id={$_SESSION['userlogin']['user_id']}&level=2");
