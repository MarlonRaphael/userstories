...
class AdminPost {
    ...
    public function ExeDelete($PostId) {
        ...
        if (!$ReadPost->getResult()):
            ...
            $readGallery->ExeRead("posts_gallery", "WHERE post_id = :id", "id={$this->Post}");
            ...
            $deleta = new Delete;
            $deleta->ExeDelete("posts_gallery", "WHERE post_id = :gbpost", "gbpost={$this->Post}");
    ...
    private function getCatParent() {
        $rCat = new Read;
        $rCat->ExeRead("categoria", "WHERE category_id = :id", "id={$this->Data['post_category']}");
        if ($rCat->getResult()):
